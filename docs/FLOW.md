...
The whole thing is encoded executably in `.claude/skills/open-edit/SKILL.md` (the golden master) and the
companion files it tells the agent to read (`TRANSCRIPTION.md`, `CUT.md`, `GENERATION.md`, `STYLE.md`, `DESIGN.md`) — this doc
...
document per chapter, gates them one at a time (`npx @veedstudio/openedit-cli gates <run-dir> --doc chapters/act-3 --no-loudnorm`) and joins the
...
before it authors any document, and authors every value out of it.
...
| cut | SCRIPT | **only when there is an edit to make** — measure the cut points (`npx @veedstudio/openedit-cli speech-probe`), assemble the kept ranges, snapped to the frame grid, in one encode with crossfaded joins (`apply-edl`), and move the existing per-word timings onto the same snapped timeline rather than transcribing again (`retime-transcript`). Splits PREP: sources are transcribed first, the assembled file is prepped after. Procedure: the CUT step of SKILL.md |
...
| design system | INLINE (authored runs only) | `runs/<key>/design/system.json` — fonts, the type ladder with each rung's own optical tracking (`pipeline/recipes/type.ts`), palette, spacing, named easings and durations, the reveal unit, the devices in play, and `donors` (the recipe ids it took mechanics from). Written FROM the run's content: `groundedIn` names the run's own files it was built from, so write it after they exist, not while waiting for them. Compose the documents from `pipeline/recipes/devices.ts`, `pipeline/design/captions.ts` and `pipeline/recipes/geometry.ts` rather than typing each graphic at its point of use. |
| design + render | SCRIPT (recipe) · INLINE (creative face-1 · remix) | default → **compiled recipe** (`npx @veedstudio/openedit-cli generate-recipe`: the generator module emits `final/template.wv` + `manifest.json` deterministically — word delays from `word-timings.json` by construction, zero tokens — then drives the gate chain: lint → `--verify` with the mechanical ladder fix loop (≤2 cycles) → `--record`); **face-1** (INLINE, orchestrator-authored: one design system from the USER'S materials, 1-2 recipe sheets as craft substrate, per `director-brief.md`; lint + `--verify` to exit 0, then `--record`); **REMIX** (inline donor blend per the brief's REMIX MODE in `runs/<key>-remix`, same gates) |
| gates | SCRIPT | `npx @veedstudio/openedit-cli gates <run-dir> [--doc <subdir>] [--audio <file>] [--no-mux] [--no-loudnorm] [--no-expect] [--no-wcag] [--no-safezones]` — lint → `--verify` (with the triaged safe-zone check) → contrast → `--record` → mux, stopping at the first failure and naming the gate. The generate-recipe command drives the same chain for a compiled recipe; every other path calls this instead of retyping it. Runs OUTSIDE any sandbox. Pass `--no-loudnorm` for one chapter of a longer piece: the level belongs to the whole film, and levelling each part on its own is how chapters end up stepping. |
| mux audio | SCRIPT | ffmpeg muxes the source audio onto the silent render and normalises it to the social delivery target (-14 LUFS / -1 dBTP / LRA 11; `--no-loudnorm` keeps the source level) → `final/out.mp4` (the deliverable) |
