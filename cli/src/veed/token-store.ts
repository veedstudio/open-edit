...
// The one place the token file location is defined; every command imports it.
export const DEFAULT_TOKEN_PATH = tokenPath();
...
// ACLs — deferred: the store lives under the user profile, whose default ACL is already owner-scoped;
// revisit with icacls if it ever moves.
...
function refreshStoredToken(
  ...
  const pending = (async (): Promise<string | null> => {
    ...
    if (!res.ok) {
      console.error(`token refresh failed (${res.status}); run: openedit login`);
      return null;
    }
...
export async function resolveToken(opts: {
  ...
  if (opts.expectedOrigin && stored.origin !== opts.expectedOrigin) {
    console.error(
      `stored login is for ${stored.origin}, not ${opts.expectedOrigin}; run: VEED_ORIGIN=${opts.expectedOrigin} openedit login`,
    );
    return null;
  }
