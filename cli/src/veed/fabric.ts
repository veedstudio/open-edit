...
//
// THERE IS NO SERVER-SIDE PER-JOB PRICE to read back, so estimateTotalCredits() below is the figure
...
// Neutral sits outside the male/female split and suits either face.
export function voiceSuitsFace(voice: FabricVoice, faceGender: FabricCharacter['gender']): boolean {
  const have = voice.gender.toLowerCase();
  return have === 'neutral' || have === faceGender;
}
...
//
// The LIPSYNC rate is MEASURED against real charges across a 20x range of durations rather than read
// from a quote: a quote carries its own conservatism, and fitting a constant to one inherits the bias
// and calls it a measurement. The SPEECH rate is the product's stated billing rule and is not
// separately corroborated — the two debits share one allowance, so a balance delta cannot split them.
// The generate command prints the measured rate on every paid run, so a drift shows before an estimate
// goes stale.
//
...
// Each preset ships a voice chosen to suit that face.
export function curatedVoiceFor(characterId: string): string | undefined {
  return FABRIC_CHARACTERS.find((c) => c.id === characterId)?.voiceId;
}
...
function toFabricVoice(v: Voice): FabricVoice {
  return {
    id: v.id,
    name: v.name,
    locale: v.locale,
...
// listVoices re-fetches the whole catalogue for every page it slices, so paging it is a dozen reads.
export async function listAllVoices(http: VeedHttp, opts: { locale: string }): Promise<FabricVoice[]> {
  return (await listVoicesRoute(http, { locale: opts.locale })).map(toFabricVoice);
}
...
export async function listVoices(
  ...
  const voices: FabricVoice[] = all.map(toFabricVoice);
