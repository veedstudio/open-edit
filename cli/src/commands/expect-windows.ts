...
export function expectWindows(argv: string[], opts: { quiet?: boolean } = {}): number {
  ...
  if (!expect.length) { say('expect-windows: no gated elements with ids — nothing to assert'); return 0; }
  if (values.write) {
    ...
    say(`expect-windows: wrote ${expect.length} expectation(s) into ${mfPath} — --verify now fails when a cue is off screen inside its own window`);
  } else {
    say(JSON.stringify({ verify: { expect } }, null, 2));
