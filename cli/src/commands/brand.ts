...
export function brandCommand(argv: string[]): number {
  ...
  if (values.brief && !problems.length) { console.log(briefFor(brand, dirname(path))); return 0; }
  for (const p of problems) console.error(`BRAND[${p.field}] ${p.message}`);
  if (problems.length) { console.error(`brand: ${problems.length} problem(s)`); return 1; }
