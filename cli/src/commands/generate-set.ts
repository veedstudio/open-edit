...
//
//   npx @veedstudio/openedit-cli generate-set --shots shots.json --key poem --workspace <id>
//   npx @veedstudio/openedit-cli generate-set --key poem --yes
//
...
export async function runSet(
  ...
  if (raw === null) {
    throw new Error(
      ...
      `  npx @veedstudio/openedit-cli generate-set --shots <file.json> --key ${args.key}`,
    );
  }
...
export async function generateSet(argv: string[]): Promise<number> {
  ...
  if (result.status === 'workspace-required' || result.status === 'no-token') return 1;
  return 0;
}
