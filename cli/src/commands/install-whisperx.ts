...
//
// Usage:
//   npx @veedstudio/openedit-cli install-whisperx            # install (or report an existing install)
//   npx @veedstudio/openedit-cli install-whisperx 3.4.3      # pin a version
//
...
//
// Installs into an isolated tool environment — uv first, pipx second — which uv/pipx manage in their
// own per-user location, so nothing here depends on a working directory. It deliberately never runs
...
export async function installWhisperx(args: string[]): Promise<number> {
  ...
  const run = (cmd: string, runArgs: string[], opts: SpawnSyncOptions = {}) => {
    const resolved = which(cmd) ?? cmd;
    if (isCmdShim(resolved)) {
      const line = [resolved, ...runArgs].map((a) => `"${a}"`).join(' ');
      return spawnSync('cmd.exe', ['/d', '/s', '/c', `"${line}"`], { stdio: 'inherit', ...opts, windowsVerbatimArguments: true });
    }
    return spawnSync(resolved, runArgs, { stdio: 'inherit', ...opts });
  };
  ...
  if (which('uv')) {
    ...
    if (run('uv', ['tool', 'install', '--python', python, spec]).status !== 0) return 1;
  } else if (which('pipx')) {
    ...
    if (run('pipx', ['install', '--python', `python${python}`, spec]).status !== 0
      && run('pipx', ['install', spec]).status !== 0) return 1;
  } else {
    console.error(`install-whisperx: neither uv nor pipx is available, and this command will not install into the system
...
Or choose VEED transcription instead, which needs no local install.`);
  ...
  if (interpreterUsable) {
    if (spawnSync(wxPython, ['-c', 'import whisperx'], { stdio: 'ignore' }).status !== 0) {
      ...
      warn('Retry pinned to another interpreter, e.g. OPEN_EDIT_WHISPERX_PYTHON=3.11 npx @veedstudio/openedit-cli install-whisperx');
      return 1;
    }
  } else {
