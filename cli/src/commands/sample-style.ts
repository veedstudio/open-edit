...
import { compiledGeneratorRelPath, generatorRelPath } from '../recipes-contract.ts';
...
// The seeded-draw law lives in ../seeded-random.ts (pinned against the repo's copy).
import { mulberry32, seedFromKey } from '../seeded-random.ts';
export { mulberry32, seedFromKey };
...
export function candidatesFor(repoRoot: string, aspect: '9:16' | '16:9'): Candidate[] {
  ...
  for (const e of readIndex(repoRoot)) {
    ...
    const hasRecipe = existsSync(join(repoRoot, compiledGeneratorRelPath(e.id))) || existsSync(join(repoRoot, generatorRelPath(e.id)));
    if (!hasRecipe) {
...
export function sampleStyle(runDir: string, opts: { seed?: number; style?: string; recipesOnly?: boolean; exclude?: string[] } = {}): StylePick {
  ...
  const style: StylePick = {
    // Content-root RELATIVE: an installed package's absolute path points into node_modules and stops
    // being true at the next update, while the run it describes outlives that by months.
    refId: pick.id, refPath: relative(repoRoot, pick.refPath), facets: pick.facets, hasRecipe: pick.hasRecipe,
...
export function readStylePick(runDir: string): Pick<StylePick, 'refId' | 'refPath' | 'hasRecipe' | 'seed'> & { facets?: Facets } {
  ...
  const known = new Set(readIndex(contentRoot()).map((e) => e.id));
  if (!known.has(raw.refId)) {
    throw new Error(`style.json in ${runDir} names refId ${JSON.stringify(raw.refId)}, which is not in the runtime index (refs/tags.json) — re-run the sample-style command for this run`);
  }
  return {
    // Re-resolved against the content answering NOW, so a run written by one install reads correctly
    // under the next; a stored absolute path is honoured as-is for runs written before the change.
    refId: raw.refId,
    refPath: isAbsolute(raw.refPath) ? raw.refPath : join(contentRoot(), raw.refPath),
