...
export function summarizeStatistics(
  ...
  // An audit that examined nothing has not passed — it has failed to run. `failingAA === 0` alone would
  // make an empty `elements` the strongest possible result.
  return {
    passed: stats.elements.length > 0 && verdicts.failingAA === 0,
...
export function wcagDetectDecision(
  ...
  // Zero audited runs needs its own sentence, not the ordinary attention one: "0 of 0 below AA" reads
  // as clean and invites an apply that cannot do anything, when the truth is that nothing was checked.
  if (before.totalRuns === 0) {
    return {
      status: 'attention',
      promoted: false,
      notes: ['WCAG AA: 0 runs audited — the statistics file names no text elements, so nothing could be verified'],
    };
  }
...
export function missingToolError(tool: 'engine' | 'remediate', p: string): string {
  switch (tool) {
    case 'engine':
      return `wcag-pass: veed-engine-cli not found at ${p} — run npx @veedstudio/openedit-cli install-engine or set VEED_ENGINE_BIN`;
    case 'remediate':
      return `wcag-pass: remediation applier not found at ${p} — it ships inside this package; set WCAG_REMEDIATE to point at a replacement`;
  }
}
...
function sh(bin: string, args: string[], okCodes: number[], quietStdout = false): void {
  // engineEnv only prepends a directory to PATH, so applying it to every tool spawned here — not
  // just the engine — is harmless.
  const r = spawnSync(bin, args, { stdio: ['ignore', quietStdout ? 'ignore' : 'inherit', 'inherit'], env: engineEnv() });
  if (r.error) {
...
export function runWcagPass(runDir: string, opts: { apply?: boolean } = {}): WcagDecision {
  ...
  // checked there.
  if (!existsSync(engineBinPath())) throw new Error(missingToolError('engine', engineBinPath()));
  ...
  if (!existsSync(wcagRemediatePath())) throw new Error(missingToolError('remediate', wcagRemediatePath()));
  // The applier is .js when published and .ts only under tsx or a WCAG_REMEDIATE override, so the flag
  // follows the file rather than the Node version. Resolved here so a .ts it cannot load fails before
  // the plan is written.
  const remediateArgs = nodeTsArgs(wcagRemediatePath());
  ...
  try {
    if (shouldRunApplier(before.passed, before.anyApplicable, hasChoice)) {
      ...
      // process.execPath, not PATH's node: the flag was chosen from this process's version.
      sh(process.execPath, [...remediateArgs, wcagRemediatePath(), ...sourceArgs, '--template-dir', finalDir], [0]);
      ...
      sh(engineBinPath(), [sibling, '--verify'], [0]);
