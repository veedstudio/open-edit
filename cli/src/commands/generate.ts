...
//   2. Confirm cost:  npx @veedstudio/openedit-cli generate --script "..." --key my-run --workspace ws-id
//   3. Spend + make:  npx @veedstudio/openedit-cli generate --key my-run --yes        (NO --script here)
//   4. If 3 broke:    npx @veedstudio/openedit-cli generate --key my-run --resume     (collects, spends nothing)
...
} from '../veed/charge-records.ts';
import {
  awaitVideo, CHARS_PER_SECOND, confirmVideo, createVideo, curatedVoiceFor, estimateRange,
  estimateTotalCredits, framingOf, FabricJobFailedError, getAllowances, listCharacters,
...
import {
  ...
  withObservation,
} from '../veed/voice-rates.ts';
import { DEFAULT_WORKSPACE_PATH, parseWorkspaceChoice, serializeWorkspaceChoice } from '../veed/workspace-store.ts';
import {
  allUnpriced, describeChoice, formatWorkspaceTable, listWorkspacesWithCredits, resolveWorkspace,
  type WorkspaceCredits,
} from '../veed/workspace.ts';
...
// Only reached when the character is not a known preset.
...
export interface Args {
  ...
  abandon?: string;
  // Spend despite an allowance reading that says it cannot cover this (a reading can be wrong).
...
export function runDirFor(key: string): string {
  return join(runsDir(), assertSafeKey(key));
}
...
const SCRIPT_WITH_YES = [
  ...
  '  npx @veedstudio/openedit-cli generate --script "spoken words" --key <run>',
  'then spend what it quoted:',
  '  npx @veedstudio/openedit-cli generate --key <run> --yes',
].join('\n');
...
export function parseArgs(argv: string[]): Args {
  ...
  // Otherwise the face brings its own, so a male preset never reads in a female voice.
  result.voice = values.voice ?? curatedVoiceFor(result.character as string) ?? DEFAULT_VOICE;
...
async function loadPending(
  ...
  if (raw === null) {
    throw new Error(
      ...
      `  npx @veedstudio/openedit-cli generate --script "spoken words" --key ${key}\n` +
...
function unreadableRecordMessage(sessionId: string, key: string): string {
  return (
    ...
    `  npx @veedstudio/openedit-cli generate --key ${key} --abandon ${sessionId}`
  );
}
...
function blockerMessage(v: ChargeVerdict, key: string): string {
  ...
  if (v.state === 'paid') {
    return (
      ...
      `  npx @veedstudio/openedit-cli generate --key ${key} --resume`
    );
  }
  if (v.state === 'charging') {
    return (
  ...
  return (
    ...
    `  npx @veedstudio/openedit-cli generate --key ${key} --abandon ${v.sessionId}`
  );
}
...
async function loadResumableJob(
  ...
  if (stranded) {
    throw new Error(
      ...
      `  npx @veedstudio/openedit-cli generate --key ${key} --abandon ${stranded.sessionId}`,
    );
  }
  throw new Error(
...
export async function run(args: Args, deps: GenerateDeps): Promise<RunResult> {
  ...
  if (!choice) {
    ...
    log(
      [
        ...
        `  npx @veedstudio/openedit-cli generate --script "spoken words" --key ${key} --workspace ${workspaces[0].id}`,
  ...
  if (pending && !pending.workspaceNamed && args.workspace !== workspaceId) {
    throw new Error(
      ...
      `  npx @veedstudio/openedit-cli generate --key ${key} --yes --workspace ${workspaceId}\n` +
  ...
  if (!pending) {
    ...
    const spendCommand = `  npx @veedstudio/openedit-cli generate --key ${key} --yes` +
...
export async function generate(argv: string[]): Promise<number> {
  ...
  // run() has already printed the workspaces and the exact command to re-run with; a non-zero exit keeps a
  // caller that chains commands from treating "nobody chose" as "done".
  if (result.status === 'workspace-required') return 1;
  return 0;
}
